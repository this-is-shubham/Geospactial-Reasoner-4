...
def refine_with_gemini(section: str, hazard: str, context: dict):
    ...
    REQUIREMENTS:
    - Always return valid JSON with fields: instruction, context, response.
    - instruction:  a citizen query in which the user mentions his area/city/state located in India where a disaster has occured ({hazard}) .
      MUST include: (1) hazard ({hazard}), (2) a real Indian city/village/state,
      and (3) either a road/landmark OR a valid 6-digit pincode. Make sure each new response has a different area/city/state in it (obv logically)
    - context: use exactly this structured JSON (already generated):
      {json.dumps(context)}
    - response: expert GIS-style disaster management plan, referencing
      hazard_data and osm_features inside context. Keep it detailed and structured.

    Background knowledge (for realism):
    {section}
    ...
    try:
        ex = json.loads(raw[start:end+1])
        ex["context"] = context  # enforce schema
        return ex
    except:
        return None
